...
urlpatterns = [
    ...
    re_path(r'^players/(?P<username>[a-zA-Z]+\w*)/scores/$', views.ScoreList.as_view(), name='score-list'),
    re_path(r'^matches/(?P<match_pk>[0-9]+)/players/(?P<username>[a-zA-Z]+\w*)/scores/$', views.ScoreDetail.as_view(), name='score-detail'),

    re_path(r'^players/(?P<username>[a-zA-Z]+\w*)/outcomes/$', views.OutcomeList.as_view(), name='outcome-list'),
